...
function setup() {
    ...
    Events.on(engine, 'collisionStart', (e) => {
        e.pairs.forEach(pair => {
            ...
            let blockHit = false;
            if (bodyA.label === 'block' && (bodyB.label === 'block' || bodyB.label === 'foundation')) {
                bodyA.hasLanded = true;
                blockHit = true;
            }
            if (bodyB.label === 'block' && (bodyA.label === 'block' || bodyA.label === 'foundation')) {
...
function draw() {
    ...
    // Hanya gerakkan kamera jika ada blok yg sudah mendarat
    ...
    fill('#424b56'); // Warna sesuai request
    noStroke();      // Solid (tanpa garis pinggir)
...
function dropBlock() {
    ...
    // PENTING: Tandai false dulu, nanti jadi true kalau kena collision
    b.hasLanded = false;
...
function drawCrane() {
    // Tali crane warna hitam dan tebal
...
function drawUI() {
    ...
    // Cek jika score mencapai 8, tampilkan modal
    if (score >= 8 && !showNextLevelModal) {
    ...
    if (showNextLevelModal) {
        ...
        textSize(18); text("Lanjut ke Level 2?", width / 2, height / 2 + 5);
        // Tombol lanjut
